...
def test_remaining_forwarders_drop_the_unusable_args():
    """The sites #1880's table did not enumerate, same defect.

    Each forwarded as `X.run(task=task, *args, **kwargs)`, which Python
    evaluates as `X.run(*args, task=task, **kwargs)` — the first splatted
    positional binds to `task`, which is already supplied by keyword, so any
    non-empty `*args` raised `TypeError: got multiple values for argument
    'task'`. The parameter is removed rather than forwarded positionally,
    because realigning the lists is what mis-slotted an argument in #1879.
    """
    import inspect
    ...
    forwarders = [
        AdvisorSwarm.batched_run,
        AdvisorSwarm.__call__,
        ReasoningAgentRouter.run,
        LLMManager._run_stream,
        LLMManager.handle_fallback_execution,
        run_agent,
        run_single_agent,
    ]
    ...
    for fn in forwarders:
        ...
        ], f"{fn.__qualname__} still advertises an unusable *args"
        assert [
            p for p in params if p.kind is p.VAR_KEYWORD
        ], f"{fn.__qualname__} lost **kwargs, so extras are unreachable"
