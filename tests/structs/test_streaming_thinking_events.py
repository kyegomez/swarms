...
# ---------------------------------------------------------------------------
# AgentRearrange streaming tests (live — require ANTHROPIC_API_KEY)
# ---------------------------------------------------------------------------

_SKIP_REARRANGE = pytest.mark.skipif(
    not __import__("os").environ.get("ANTHROPIC_API_KEY"),
    reason="ANTHROPIC_API_KEY required for AgentRearrange streaming tests",
)

REARRANGE_MODEL = "claude-sonnet-4-6"
REARRANGE_TASK = "What is 2+2? Reply with just the number."


def _thinking_agent(name: str) -> Agent:
    return Agent(
        agent_name=name,
        model_name=REARRANGE_MODEL,
        thinking_tokens=1024,
        max_loops=1,
        print_on=False,
        verbose=False,
...
@_SKIP_REARRANGE
class TestAgentRearrangeStreamingLive:
    @pytest.mark.asyncio
    async def test_sequential_thinking_events_have_agent_field(self):
        a = _thinking_agent("First")
        b = _thinking_agent("Second")
        ...
        collected = []
        async for evt in rearrange.arun_stream(
            REARRANGE_TASK, with_events=True
        ):
        ...
        thinking_evts = [
            e
            for e in collected
            if isinstance(e, dict) and e.get("type") == "thinking"
        ]
        assert len(thinking_evts) > 0, "no thinking events received"
        assert all("agent" in e for e in thinking_evts)

    @pytest.mark.asyncio
    async def test_sequential_chunks_aggregation_excludes_thinking(
        self,
    ):
        a = _thinking_agent("First")
        b = _thinking_agent("Second")
        ...
        async for _ in rearrange.arun_stream(
            REARRANGE_TASK, with_events=True
        ):
            pass

        history = rearrange.conversation.return_history_as_string()
        thinking_evts_in_history = (
            "<thinking>" in history or "reasoning_content" in history
        )
        assert not thinking_evts_in_history or "4" in history

    @pytest.mark.asyncio
    async def test_sequential_event_ordering_thinking_before_content(
        self,
    ):
        a = _thinking_agent("First")
        b = _thinking_agent("Second")
        ...
        types = [
            e.get("type") for e in collected if isinstance(e, dict)
        ]
        if "thinking" in types and "token" in types:
            assert types.index("thinking") < types.index("token")

    @pytest.mark.asyncio
    async def test_sequential_three_agent_thinking_events(self):
        a = _thinking_agent("First")
        b = _thinking_agent("Second")
        c = _thinking_agent("Third")
        rearrange = AgentRearrange(
            agents=[a, b, c],
        ...
        assert len(thinking_evts) > 0, "no thinking events received"
        agent_names = {e["agent"] for e in thinking_evts}
