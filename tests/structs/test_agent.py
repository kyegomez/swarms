...
class TestGeneratedAgentName:
    """An omitted agent_name must produce a unique per-instance name.

    It defaulted to the literal "swarm-worker-01", so every agent that did
    not set one shared it. Anything keyed on the name then collided: three
    unnamed workers in run_agents_concurrently(return_agent_output_dict=True)
    collapsed to a single entry, and MEMORY.md is keyed on it too.
    """

    @staticmethod
    def _agent(**kwargs):
        return _patched_agent(
            kwargs.pop("agent_name", None), **kwargs
        )

    def test_omitted_name_is_generated_per_instance(self):
        """The generator must run in __init__, not in the signature.

        A generated default would be evaluated once at import and baked into
        __defaults__, handing every agent the same name -- the defect
        test_component_ids.py pins for component ids.
        """
        names = {self._agent().agent_name for _ in range(5)}

        assert len(names) == 5
        assert (
            Agent.__init__.__defaults__ is None
            or "agent-" not in str(Agent.__init__.__defaults__ or "")
        )

    def test_generated_name_has_the_documented_shape(self):
        assert re.fullmatch(
            r"agent-[a-z0-9]{5}", self._agent().agent_name
        )

    def test_explicit_name_is_untouched(self):
        agent = self._agent(agent_name="Researcher")

        assert agent.agent_name == "Researcher"
        assert agent.name_autogenerated is False

    def test_name_mirrors_agent_name(self):
        """`self.name` read the raw parameter, which is now None when
        omitted, so it had to follow the resolved value instead."""
        agent = self._agent()

        assert agent.name == agent.agent_name
        assert agent.name_autogenerated is True
