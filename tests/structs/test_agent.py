...
class TestAutonomousAgentLoop:
    ...
    def test_builtin_file_tool_reads_through_the_agent(self):
        """The built-in tools take the agent, not the loop that dispatches them."""
        prompts = []

        with tempfile.NamedTemporaryFile(
            "w", suffix=".txt", delete=False
        ) as handle:
            handle.write("file-contents-here")
            path = handle.name
        ...
        assert any(
            "file-contents-here" in r for r in results
