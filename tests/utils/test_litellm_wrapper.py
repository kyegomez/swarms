...
def test_count_tokens_is_cached_and_bounded():
    """count_tokens runs on the full, growing transcript every turn.

    22 call sites re-encode the same prefix continuously, which is the engine
    behind the O(n^2) conversation cost (#1735). The cache is keyed by digest
    rather than by the text: an lru_cache over the raw string would pin
    hundreds of full transcripts in memory, which for a long run costs more
    than the work it saves.
    """
    import time

    from swarms.utils import litellm_tokenizer as lt

    text = "the quick brown fox jumps over the lazy dog. " * 2000

    lt._TOKEN_COUNT_CACHE.clear()

    start = time.perf_counter()
    first = lt.count_tokens(text)
    cold = time.perf_counter() - start

    start = time.perf_counter()
    second = lt.count_tokens(text)
    warm = time.perf_counter() - start

    # Same answer, and the second call did not re-encode.
    assert first == second
    assert warm < cold

    # Distinct inputs are not conflated.
    assert lt.count_tokens("hello world") != lt.count_tokens(
        "hello world hello world"
    )

    # The cache cannot grow without bound.
    for i in range(lt._TOKEN_COUNT_CACHE_MAX + 50):
        lt.count_tokens(f"distinct text {i}")
    assert len(lt._TOKEN_COUNT_CACHE) <= lt._TOKEN_COUNT_CACHE_MAX

    # Counts are per model, so two models never share an entry.
    assert lt._cache_key("x", "model-a") != lt._cache_key(
        "x", "model-b"
    )
