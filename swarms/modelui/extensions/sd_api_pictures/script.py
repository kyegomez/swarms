...
SD_models = [
    "NeverEndingDream"
]  # TODO: get with http://{address}}/sdapi/v1/sd-models and allow user to select
...
def ui():
    ...
    with gr.Accordion("Parameters", open=True, elem_classes="SDAP"):
        with gr.Row():
            ...
            with gr.Column(scale=1, min_width=300):
                manage_VRAM = gr.Checkbox(
                    value=params["manage_VRAM"], label="Manage VRAM"
                )
                save_img = gr.Checkbox(
                    value=params["save_img"],
        ...
        with gr.Accordion("Generation parameters", open=False):
            ...
            with gr.Row():
                with gr.Column():
                    width = gr.Slider(
                        64, 2048, value=params["width"], step=64, label="Width"
                    )
                    height = gr.Slider(
                        64, 2048, value=params["height"], step=64, label="Height"
                    )
            ...
            with gr.Row():
                ...
                with gr.Column() as hr_options:
                    restore_faces = gr.Checkbox(
                        value=params["restore_faces"], label="Restore faces"
                    )
                    enable_hr = gr.Checkbox(
                        value=params["enable_hr"], label="Hires. fix"
                    )
            with gr.Row(
                ...
                hr_upscaler = gr.Textbox(
                    placeholder=params["hr_upscaler"],
    ...
    address.change(
        lambda x: params.update({"address": filter_address(x)}), address, None
    )
    ...
    textgen_prefix.change(
        lambda x: params.update({"textgen_prefix": x}), textgen_prefix, None
    )
    ...
    sampler_name.change(
        lambda x: params.update({"sampler_name": x}), sampler_name, None
    )
