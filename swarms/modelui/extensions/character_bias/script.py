...
def ui():
    ...
    dropdown_string = gr.Dropdown(
        ...
        label="Character bias",
        info='To edit the options in this dropdown edit the "bias_options.txt" file',
    )
    use_custom_string = gr.Checkbox(
        value=False, label="Use custom bias textbox instead of dropdown"
    )
    custom_string = gr.Textbox(
        value="",
        placeholder="Enter custom bias string",
        label="Custom Character Bias",
        info="To use this textbox activate the checkbox above",
    )
