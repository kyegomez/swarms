...
def output_modifier(string, state):
    ...
    if string == "":
        ...
        output_file = Path(
            f'extensions/silero_tts/outputs/{state["character_menu"]}_{int(time.time())}.wav'
        )
...
def voice_preview(string):
    ...
    prosody = (
        f"<prosody rate=\"{params['voice_speed']}\" pitch=\"{params['voice_pitch']}\">"
    )
    silero_input = f"<speak>{prosody}{xmlesc(string)}</prosody></speak>"
    model.save_wav(
        ssml_text=silero_input,
...
def language_change(lang):
    ...
    params.update(
        {
            "language": lang,
            "speaker": languages[lang]["default_voice"],
            "model_id": languages[lang]["model_id"],
        }
    )
    return gr.update(
        choices=languages[lang]["voices"], value=languages[lang]["default_voice"]
    )
    ...
    with gr.Accordion("Silero TTS"):
        ...
        with gr.Row():
            ...
            voice = gr.Dropdown(
                ...
                choices=languages[params["language"]]["voices"],
                label="TTS voice",
            )
        with gr.Row():
            v_pitch = gr.Dropdown(
