...
def split_sentences(text: str, cutoff_len: int):
    ...
    for char in text:
        ...
        if (
            any(sentence.endswith(delimiter) for delimiter in delimiters)
            and not (prev_char.isupper() and len(sentence) >= 3 and sentence[-3] != " ")
            and not any(
                sentence.endswith(abbreviation) for abbreviation in abbreviations
            )
        ):
...
def precise_cut(
    ...
    for index, item in enumerate(sentences):
        ...
        if totalLength + item["size"] < max_cut and not currentSentence.endswith(
            EOSX_str
        ):
        ...
        for edge_idx in edgeindex:
            ...
            for item in sentences[edge_idx:]:
                if totalLength + item["size"] < max_cut:
                    ...
                    if (
                        currentSentence.endswith(EOSX_str)
                        and len(currentSentence.strip()) > min_chars_cut
                    ):
                        sentencelist.append(currentSentence.strip())
                    # otherwise don't cross hard cut
                    elif (
                        EOSX_str not in currentSentence
                        and len(currentSentence.strip()) > min_chars_cut
                    ):
                        sentencelist.append(currentSentence.strip())
    ...
    if debug_slicer:
        ...
        sentencelist_dict = {
            index: sentence for index, sentence in enumerate(sentencelist)
        }
    ...
    for i in range(len(sentences)):
        ...
        if i >= advancing_to:
            for k in range(i, len(sentences)):
                ...
                if (
                    totalLength + current_length <= max_cut
                    and not currentSentence.endswith(EOSX_str)
                ):
...
def download_file_from_url(
    url, overwrite, output_dir_in, valid_extensions={".txt", ".json"}
):
    try:
