...
if hasattr(torch.utils.checkpoint, "noop_context_fn"):
    ...
    def my_checkpoint(
        ...
        *args,
        ...
        ] = torch.utils.checkpoint.noop_context_fn,
        ...
        **kwargs,
        ...
        if use_reentrant:
            if (
                context_fn is not torch.utils.checkpoint.noop_context_fn
            ...
            return torch.utils.checkpoint.CheckpointFunction.apply(
...
params = {"display_name": "Training PRO", "is_tab": True}
...
PARAMETERS = [
    ...
    "min_chars",
    "report_to",
    "precize_slicing_overlap",
    "add_eos_token_type",
    "save_steps_under_loss",
    "add_bos_token",
    "training_projection",
    "sliding_window",
    "warmup_ratio",
...
def ui():
    with gr.Tab("Train LoRA", elem_id="lora-train-tab"):
        ...
        with gr.Row():
            with gr.Column():
                # YY.MM.DD
                gr.Markdown(
                    "`Ver: 23.10.20` This is enhanced version of QLora Training. [Maintained by FP](https://github.com/FartyPants/Training_PRO/tree/main)"
                )
                ...
                with gr.Row():
                    with gr.Column(scale=5):
                        with gr.Row():
                            copy_from = gr.Dropdown(
                                ...
                                value="None",
                                choices=get_available_loras_local(
                                    non_serialized_params["Lora_sortedByTime"]
                                ),
                    ...
                    with gr.Column():
                        sort_byTime = gr.Checkbox(
                            label="Sort list by Date",
                            value=False,
                            info="Sorts Loras by date created.",
                ...
                with gr.Row():
                    with gr.Column():
                        ...
                        batch_size = gr.Slider(
                            ...
                            maximum=1024,
                            step=4,
                            info="Now Replaced with Gradient accumulation. Keeping it for sake of old saved data",
                        )
                        micro_batch_size = gr.Slider(
                            label="True Batch Size",
                            value=4,
                            minimum=1,
                            maximum=128,
                            step=1,
                            info="Specifies how many text blocks per step will be trained. The higher value, the better the concept of training will be, but it requires more GPU memory and it reduces speed.",
                        )
                        grad_accumulation = gr.Slider(
                            ...
                            maximum=256,
                            step=1,
                            info="Virtually multiplies the Batch Size by averaging the learning over more than one step. VRAM friendly. Evens out loss fluctuations but can also degrade training fidelity.",
                        )

                    with gr.Column():
                        stop_at_loss = gr.Slider(
                            label="Stop at loss (Can be changed during training)",
                            minimum=0.0,
                            maximum=3.0,
                            step=0.1,
                            value=0.00,
                            info="The process will automatically stop once the desired loss value is reached.",
                        )
                        gr.Markdown(" ")
                        epochs = gr.Number(
                            label="Epochs",
                        ...
                        lr_scheduler_type = gr.Dropdown(
                            ...
                            info="Learning rate scheduler - defines how the learning rate changes over time. Custom schedulers: FP_low_epoch_annealing, FP_half_time_annealing, FP_raise_fall_creative (see README)",
                            elem_classes=["slim-dropdown"],
                        )
                    ...
                    with gr.Row():
                        with gr.Column():
                            save_steps = gr.Number(
                                ...
                                value=0,
                                info="A checkpoint will be saved every n steps and at each Epoch boundary. (0 = OFF)",
                            )
                        with gr.Column():
                            save_steps_under_loss = gr.Slider(
                                label="Save at 10% Loss change",
                                value=1.8,
                                minimum=0.0,
                                maximum=3.0,
                                step=0.1,
                                info="Saves checkpoints at (or bellow) this loss and then each time loss falls by at least 10% This works independently from 'Save every n steps'",
                            )
                    with gr.Row():
                    ...
                    with gr.Row():
                        with gr.Column():
                            warmup_steps = gr.Number(
                                label="Warmup Steps",
                                value=100,
                                info="Number of max steps used for a linear warmup. Reduces early over-fitting by the first training blocks. Value has precedent over Warmup Ratio. Aligns to the closest multiple of graddient accumulation",
                            )
                            warmup_ratio = gr.Slider(
                                ...
                                step=0.025,
                                value=0.0,
                                info="Ratio of total training steps that will be used for a linear warmup. It applies only if Warmup Step is 0.",
                            )
                            neft_noise_alpha = gr.Slider(
                                label="NEFtune noise scale",
                                minimum=0.0,
                                maximum=15,
                                step=1,
                                value=0.0,
                                info="Add noise to the training to improve generalization. [0 - OFF, Starting value to experiment: 5]",
                            )
                            training_projection = gr.Radio(
                                value=train_choices[4],
                                label="LLaMA Target Projections",
                                info="Change the targets (LORA is typically q-v)",
                        ...
                        with gr.Column():
                            ...
                            add_bos_token = gr.Checkbox(
                                label="Add BOS token",
                                value=True,
                                info="Adds BOS token for each dataset item",
                            )
                            add_eos_token = gr.Checkbox(
                                label="Add EOS token",
                                value=False,
                                info="Adds EOS token for each dataset item",
                            )
                            add_eos_token_type = gr.Dropdown(
                                label="EOS placement (Text file)",
                                choices=["Every Block", "Hard Cut Blocks Only"],
                                value="Every Block",
                                info="",
                ...
                with gr.Tab(label="Formatted Dataset"):
                    with gr.Row():
                        with gr.Column():
                            with gr.Row():
                                dataset = gr.Dropdown(
                                    choices=get_datasets("training/datasets", "json"),
                                    value="None",
                                    label="Dataset",
                        ...
                        with gr.Column():
                            with gr.Row():
                                format = gr.Dropdown(
                                    choices=get_datasets("training/formats", "json"),
                                    value="None",
                                    label="Data Format",
                ...
                with gr.Tab(label="Text file"):
                    with gr.Row():
                        raw_text_file = gr.Dropdown(
                            choices=get_datasets("training/datasets", "txt"),
                            value="None",
                            label="Text file",
                            info="The text file to use for training.",
                            elem_classes=["slim-dropdown"],
                        )
                        create_refresh_button(
                            raw_text_file,
                    ...
                    with gr.Row():
                        with gr.Column():
                            precize_slicing_overlap = gr.Checkbox(
                                label="Add Overlapping blocks", value=True
                            )
                            sliding_window = gr.Checkbox(
                                label="DEMENTOR Long-form Learning by FP (Highly Experimental, use low epochs)",
                                value=False,
                                info="Deep Memorization Enforcement Through Overlapping and Repetition. (I named it, so shush). Special process for learning long-form text using low amount of epochs.",
                            )
                        ...
                        with gr.Column():
                            ...
                            min_chars = gr.Number(
                                ...
                                value=0,
                                info="Ignore Text blocks that have less or equal characters than this number.",
                            )
                with gr.Tab(label="URL"):
                    with gr.Row():
                        with gr.Column():
                            download_file_url = gr.Textbox(
                                label="Download JSON or txt file to datasets (or formats) folder",
                                value="",
                                info="The URL of a file to download. If on github, make sure you get url of the raw file (https://raw.githubusercontent.com/...). If huggin face, make sure the url has /resolve/ in it not /blob/",
                            )
                            with gr.Row():
                                download_check_overwrite = gr.Checkbox(
                                    label="Overwrite",
                                    value=False,
                                    info="Overwrite if file exist",
                                )
                                download_folder = gr.Radio(
                                    label="Destination",
                                    value="training/datasets",
                                    choices=["training/datasets", "training/formats"],
                                    interactive=True,
                                )
                            download_button = gr.Button("Download")
                            download_status = gr.Textbox(
                                label="Download Status", value="", interactive=False
                            )
                with gr.Row():
                    with gr.Column():
                        with gr.Row():
                            cutoff_len = gr.Slider(
                                label="Chunk Length (Cutoff Length)",
                                minimum=32,
                                maximum=2048,
                                value=256,
                                step=32,
                                info="The maximum length of a chunk (in tokens). Applies to both JSON dataset and text files. Higher values require much more VRAM.",
                            )
                with gr.Row():
                    with gr.Column():
                        check_dataset_btn = gr.Button(
                            "Verify Dataset/Text File and suggest data entries"
                        )
        ...
        with gr.Row():
            with gr.Column():
                ...
                evaluate_text_file = gr.Dropdown(
                    ...
                    + get_datasets("training/datasets", "txt")[1:],
                    value="wikitext",
                    label="Input dataset",
                    info="The text file on which the model will be evaluated. The first options are automatically downloaded: wikitext, ptb, and ptb_new. The next options are your local text files under training/datasets.",
                )
                with gr.Row():
                    with gr.Column():
    ...
    def fix_old_version(batch_size_val, micro_batch_size_val, grad_accumulation_val):
        if batch_size_val > 0:
            ...
            print(
                f"Using Old version of Batch Size ({batch_size_val}) to set Gradient Accumulation: {gradient_acc}"
            )
            return gradient_acc
    ...
    copy_from.change(
        partial(do_copy_params, all_params=all_params), copy_from, all_params
    ).then(
        fix_old_version,
        [batch_size, micro_batch_size, grad_accumulation],
        grad_accumulation,
    )
    ...
    def trigger_save_checkpoint():
        ...
        if non_serialized_params["training_loop"]:
            print(
                "Queue: [Save checkpoint] Checkpoint will be saved after the current step is finished."
            )
        else:
    ...
    def update_button():
        return gr.Button.update(
            "[Checkpoint in Queue]", variant="stop", interactive=True
        )

    def update_button2():
        time.sleep(1.0)
        return gr.Button.update(
            "Queue Checkpoint Now", variant="secondary", interactive=True
        )

    save_chackpoint_now.click(trigger_save_checkpoint, None, None).then(
        update_button, None, save_chackpoint_now
    ).then(update_button2, None, save_chackpoint_now)
    ...
    def check_dataset(
        ...
        if raw_text_file not in ["None", ""]:
            ...
            if sliding_window:
                text_chunks = sliding_block_cut(
                    raw_text,
                    min_chars,
                    False,
                    cutoff_len,
                    hard_cut_string,
                    non_serialized_params["debug_slicer"],
                )
            else:
        ...
        if total_blocks > 0:
            ...
            warmup_steps_suggest = min(
                int(min_warm * grad_accumulation), int(math.ceil(number_ofSteps * 0.1))
            )
            ...
            result += (
                f"Warmup steps: {warmup_steps_suggest} (Current: {int(warmup_steps)})"
            )
    ...
    sort_byTime.change(
        lambda x: non_serialized_params.update({"Lora_sortedByTime": x}),
        sort_byTime,
        None,
    ).then(reload_lora, None, copy_from)
    # debug_slicer.change(lambda x: non_serialized_params.update({"debug_slicer": x}), debug_slicer, None)

    def update_dataset():
        return gr.update(choices=get_datasets("training/datasets", "json")), gr.update(
            choices=get_datasets("training/datasets", "txt")
        )

    download_button.click(
        download_file_from_url,
        [download_file_url, download_check_overwrite, download_folder],
        download_status,
    ).then(update_dataset, None, [dataset, raw_text_file])
...
def do_train(
    ...
    report_to: str,
    precize_slicing_overlap: bool,
    add_eos_token_type: str,
    save_steps_under_loss: float,
    add_bos_token: bool,
    training_projection: str,
    sliding_window: bool,
    warmup_ratio: float,
    grad_accumulation: int,
    neft_noise_alpha: float,
):
    ...
    non_serialized_params.update(
        {"save_steps_under_loss": save_steps_under_loss + 0.01}
    )
    ...
    if hasattr(torch.utils.checkpoint, "noop_context_fn"):
        ...
        if old_checkpoint_signature.parameters == my_checkpoint_signature.parameters:
            print(
                f"{RED}Overriding Torch checkpoint function to avoid repeated 'use_reentrant not explicitly set' warnings{RESET}"
            )
    ...
    if raw_text_file not in ["None", ""]:
        ...
        train_data = Dataset.from_list(
            [tokenize(x, add_EOS_to_all, add_bos_token) for x in text_chunks]
        )
        if add_EOS_to_all:
    ...
    if training_projection == train_choices[0]:
        model_to_lora_modules[model_id] = [
            "gate_proj",
            "down_proj",
            "up_proj",
            "q_proj",
    ...
    try:
        ...
        if (
            ...
            print(
                f" + Continue Training on {RED}{lora_file_path}/adapter_model.bin{RESET}"
            )
                ...
                with open(f"{lora_file_path}/training_log.json", "r") as json_file:
                    ...
                    for key, value in json_ilog.items():
                        if key == "current_steps":
                            non_serialized_params.update(
                                {"checkpoint_offset": int(value + 1)}
                            )
                            print(
                                f" + Checkpoints will be saved with offset: {RED}{non_serialized_params['checkpoint_offset']}{RESET}"
                            )
                        if key == "epoch":
            ...
            if Path(f"{lora_file_path}/training_graph.json").is_file():
                try:
                    with open(
                        f"{lora_file_path}/training_graph.json", "r"
                    ) as json_file:
    ...
    class Callbacks(transformers.TrainerCallback):
        def on_step_begin(
            ...
            **kwargs,
            ...
            if WANT_INTERRUPT:
                ...
                current_steps_offset = (
                    tracked.current_steps + non_serialized_params["checkpoint_offset"]
                )
                ...
                if non_serialized_params["save_checkpoint_now"]:
                    ...
                    non_serialized_params.update({"save_checkpoint_now": False})
                    print(
                        f"\033[1;31;1mSave Checkpoint manually trigerred.\033[0;37;0m"
                    )
                    folder_save = f"checkpoint-{current_steps_offset}-user"

                patience = (
                    3  # Set the number of consecutive steps for tracking stability
                )
                ...
                if (
                    current_loss < non_serialized_params["save_steps_under_loss"]
                    and current_loss > 0
                    and state.global_step > min_steps
                ):
                    ...
                    if current_stability >= patience:
                        ...
                        non_serialized_params.update(
                            {"save_steps_under_loss": new_save}
                        )

                        folder_save = (
                            f"checkpoint-{current_steps_offset}-loss-{loss_str}"
                        )
                        force_save = True
                ...
                if (
                    actual_save_steps > 0
                    and current_epoch_int > non_serialized_params["save_epochs"]
                    and state.global_step > min_steps
                ):
                    current_epoch_offset = current_epoch_int

                    if non_serialized_params["epoch_offset"] > 0:
                        current_epoch_offset = current_epoch_int + round(
                            non_serialized_params["epoch_offset"], 2
                        )

                    ep_off_str = f"{current_epoch_offset}"
                    ep_off_str = ep_off_str.replace(".", "_")
                    folder_save = (
                        f"checkpoint-{current_steps_offset}-epoch-{ep_off_str}"
                    )
                ...
                if force_save:
                    ...
                    print(
                        f"\033[1;30;40mStep: {tracked.current_steps:6} \033[0;37;0m Saved: [{folder_save}]"
                    )
                    # Save log
                    with open(
                        f"{lora_file_path}/{folder_save}/training_log.json",
                        "w",
                        encoding="utf-8",
                    ) as file:
                    ...
                    with open(
                        f"{lora_file_path}/{folder_save}/training_prompt.json",
                        "w",
                        encoding="utf-8",
                    ) as file:
        ...
        def on_log(
            ...
            **kwargs,
            ...
            current_epoch_offset = (
                train_log.get("epoch", 0.0) + non_serialized_params["epoch_offset"]
            )
            ...
            if non_serialized_params["checkpoint_offset"] > 0:
                print(
                    f"\033[1;30;40mStep: {tracked.current_steps:6} [+{non_serialized_params['checkpoint_offset']}] \033[0;37;0m",
                    end="",
                )
            else:
                print(
                    f"\033[1;30;40mStep: {tracked.current_steps:6} \033[0;37;0m", end=""
                )
    ...
    if gradient_accumulation_max < gradient_accumulation_steps:
        print(
            f"{RED}WARNING:{RESET} Current gradient accumulation is {RED}too high{RESET} for the amount of training data."
        )
        print(
            f"Gradient accumulation: {gradient_accumulation_steps} should be less than: {gradient_accumulation_max}. {RED}This could crash Accelerate/Transformers{RESET}"
        )
        # min_batchSize = sample_req*micro_batch_size
        print(f"Preferable fix: {RED}Increase the size of dataset{RESET}")
        print(
            f"... or Decrerase Gradient Accumulation {RED}{gradient_accumulation_steps}{RESET} to below {GREEN}{gradient_accumulation_max}{RESET}"
        )
        gradient_accumulation_steps = max(1, gradient_accumulation_max - 1)
        print(
            f"Last resort fix for this run: Lowering Gradient accumulation to {GREEN}{gradient_accumulation_steps}{RESET} [Good luck]"
        )

    else:
        print(
            f"Data Size Check: Gradient accumulation: {YELLOW}{gradient_accumulation_steps}{RESET} <= Blocks/Batch {gradient_accumulation_max} ... {GREEN}[OK]{RESET}"
        )
    ...
    print(
        f"Training '{model_id}' model using {YELLOW}({projections_string}){RESET} projections"
    )

    if lora_all_param > 0:
        print(
            f"Trainable params: {lora_trainable_param:,d} ({RED}{100 * lora_trainable_param / lora_all_param:.4f} %{RESET}), All params: {lora_all_param:,d} (Model: {model_all_params:,d})"
        )
    ...
    if non_serialized_params["checkpoint_offset"] > 0:
        train_log.update(
            {"last_run_steps_offset": non_serialized_params["checkpoint_offset"]}
        )
        train_log.update(
            {"last_run_epoch_offset": non_serialized_params["epoch_offset"]}
        )

    if non_serialized_params["checkpoint_offset"] > 0:
        print(
            f"Continue training on {RED}previous adapter{RESET} from epoch: {RED}{non_serialized_params['epoch_offset']}{RESET}"
        )
    ...
    while thread.is_alive():
        ...
        if WANT_INTERRUPT:
            ...
            if stop_at_loss != non_serialized_params["stop_at_loss"]:
                ...
                print(
                    f"Stop at loss changed {RED}(Auto-Stop at: {stop_at_loss}){RESET}"
                )
