...
custom_scheduler_params = {
    ...
    "ramp_down_ratio": 1.0,
    "current_loss": 0.0,
    "dynamic_scheduler_stop": False,
    "calc_ramp_down_at_step": 0,
    "calc_num_training_steps": 0,
}
...
def _get_fp_half_schedule_with_warmup_lr_lambda(
    current_step: int,
    *,
    num_warmup_steps: int,
    num_training_steps: int,
    num_firstepoch_steps: int,
):
    ...
    progress = float(current_step - half_steps) / float(
        max(1, num_training_steps - half_steps)
    )
    num_cycles = 0.5
    return max(
...
def _get_fp_cosine_raise_and_fall_lr_lambda(
    current_step: int,
    *,
    num_warmup_steps: int,
    num_training_steps: int,
...
def _get_fp_cosine_schedule_with_warmup_lr_lambda(
    ...
    *,
    ...
    progress = float(current_step - num_firstepoch_steps) / float(
        max(1, num_training_steps - num_firstepoch_steps)
    )
    num_cycles = 0.5
    return max(
        0.0, 0.5 * (1.0 + math.cos(math.pi * float(num_cycles) * 2.0 * progress))
...
def custom_cosine_scheduler_with_warmup(
    optimizer, num_warmup_steps, num_training_steps, num_firstepoch_steps, last_epoch=-1
):
    """
    Args:
...
def custom_half_scheduler_with_warmup(
    optimizer, num_warmup_steps, num_training_steps, num_firstepoch_steps, last_epoch=-1
):
    """
    Args:
...
def custom_raise_fall_scheduler_with_warmup(
    optimizer, num_warmup_steps, num_training_steps, num_firstepoch_steps, last_epoch=-1
):
    """
    Args:
...
class FPSchedulerTrainer(transformers.Trainer):
    ...
    def create_scheduler(
        ...
        num_firstepoch_steps_acc = (
            num_firstepoch_steps * self.args.gradient_accumulation_steps
        )
        num_training_steps_acc = (
        ...
        print(
            f"Warm-up steps aligned to Gradient accumulation ({self.args.gradient_accumulation_steps}) = {num_warmup_acc} actual warmup steps"
        )
        if self.args.lr_scheduler_type == "cosine":
            ...
            if num_warmup_acc > num_firstepoch_steps_acc:
                print(
                    f"\033[1;31;1mWARNING: The number of warmup steps is set too high! It will be clamped to 1 epoch, essentially going from warmup to annealing.\033[0;37;0m"
                )
                print(
                    f"FP Scheduler Warmup: 0-[{num_warmup_acc_min}], Hold [{num_warmup_acc_min}]-{num_firstepoch_steps_acc}, Annealing {num_firstepoch_steps_acc}-{num_training_steps_acc}"
                )
            else:
                print(
                    f"FP Scheduler Warmup: 0-{num_warmup_acc_min}, Hold {num_warmup_acc_min}-{num_firstepoch_steps_acc}, Annealing {num_firstepoch_steps_acc}-{num_training_steps_acc}"
            ...
            if num_warmup_acc > half_step_acc:
                print(
                    f"\033[1;31;1mWARNING: The number of warmup steps is set too high! It will be clamped to half of all epochs, essentially going from warmup to annealing in the middle.\033[0;37;0m"
                )
                print(
                    f"FP Scheduler Warmup: 0-[{num_warmup_acc_min}], Hold [{num_warmup_acc_min}]-{half_step_acc}, Annealing {half_step_acc}-{num_training_steps_acc}"
                )
            else:
                print(
                    f"FP Scheduler Warmup: 0-{num_warmup_acc_min}, Hold {num_warmup_acc_min}-{half_step_acc}, Annealing {half_step_acc}-{num_training_steps_acc}"
            ...
            print(
                f"Scheduler Raise: 0-{half_step_acc}, Fall {half_step_acc}-{num_training_steps_acc}"
            )
