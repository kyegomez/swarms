...
class LogitsBiasProcessor(LogitsProcessor):
    ...
    def __call__(
        ...
        if self.logit_bias:
            ...
            debug_msg(
                " max/min ", float(torch.max(logits[0])), float(torch.min(logits[0]))
            )
    ...
    if logit_bias:  # {str: float, ...}
        ...
        try:
            ...
            for logit, bias in logit_bias.items():
                for x in encode(encoder.decode([int(logit)]), add_special_tokens=False)[
                    0
                ]:
...
def convert_history(history):
    ...
    return (
        user_input,
        system_message,
        {"internal": chat_dialogue, "visible": copy.deepcopy(chat_dialogue)},
    )
...
def chat_completions_common(body: dict, is_legacy: bool = False, stream=False) -> dict:
    if body.get("functions", []):
        raise InvalidRequestError(
            message="functions is not supported.", param="functions"
        )

    if body.get("function_call", ""):
        raise InvalidRequestError(
            message="function_call is not supported.", param="function_call"
        )
    ...
    for m in messages:
        if "role" not in m:
            ...
            raise InvalidRequestError(
                message="role: function is not supported.", param="messages"
            )
        if "content" not in m:
            raise InvalidRequestError(
    ...
    instruction_template = (
        body["instruction_template"] or shared.settings["instruction_template"]
    )
    instruction_template = (
        "Alpaca" if instruction_template == "None" else instruction_template
    )
    (
        name1_instruct,
        name2_instruct,
    ...
    name1, name2, _, greeting, context, _, _ = load_character_memoized(
        character, name1, "", instruct=False
    )
    ...
    def chat_streaming_chunk(content):
        ...
        if logprob_proc:  # not official for chat yet
            top_logprobs = convert_logprobs_to_tiktoken(
                model=requested_model, logprobs=logprob_proc.token_alternatives
            )
    ...
    for a in generator:
        ...
        if stream:
            ...
            if (
                not new_content or chr(0xFFFD) in new_content
    ...
    stop_reason = "stop"
    if (
        token_count + completion_token_count >= generate_params["truncation_length"]
        or completion_token_count >= generate_params["max_new_tokens"]
    ):
        stop_reason = "length"
...
def completions_common(body: dict, is_legacy: bool = False, stream=False):
    ...
    if not stream:
        ...
        if isinstance(prompt_arg, str) or (
            isinstance(prompt_arg, list) and isinstance(prompt_arg[0], int)
        ):
        ...
        for idx, prompt in enumerate(prompt_arg, start=0):
            ...
            if (
                ...
                >= generate_params["truncation_length"]
                or completion_token_count >= max_tokens
            ):
                stop_reason = "length"
            ...
            respi = {
                ...
                "logprobs": {"top_logprobs": [logprob_proc.token_alternatives]}
                if logprob_proc
                else None,
            }
        ...
        if isinstance(prompt, list):
            if prompt and isinstance(prompt[0], int):
                ...
                raise InvalidRequestError(
                    message="API Batched generation not yet supported.",
                    param=prompt_str,
                )
