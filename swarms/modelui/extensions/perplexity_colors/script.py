...
class PerplexityLogits(LogitsProcessor):
    ...
    def __call__(self, input_ids, scores):
        ...
        log_probs = torch.nan_to_num(
            torch.log(probs)
        )  # Note: This is to convert log(0) nan to 0, but probs*log_probs makes this 0 not affect the perplexity.
        ...
        if len(self.selected_probs) > 0:
            ...
            if self.verbose:
                ...
                print(
                    ...
                    [
                        ...
                        for token_id in self.top_token_ids_list[-1][0]
                    ],
                )
                print(
                    "Probs:",
            ...
            self.selected_probs.append(
                1.0
            )  # Placeholder for the last token of the prompt
        ...
        if self.verbose:
            ...
            print(
                f"PPL: Token after {shared.tokenizer.decode(last_token_id)}\t{perplexity:.2f}\t{pplbar}"
            )
...
def output_modifier(text):
    ...
    top_tokens_list = [
        [shared.tokenizer.decode(token_id) for token_id in top_token_ids[0]]
        for top_token_ids in top_token_ids_list
    ]
    ...
    end_part = (
        "</div></div>" if params["probability_dropdown"] else "</span>"
    )  # Helps with finding the index after replacing part of the text.
        ...
        if token in text[i:]:
            if params["probability_dropdown"]:
                text = text[:i] + text[i:].replace(
                    token,
                    add_dropdown_html(token, color, top_tokens, top_probs[0], ppl),
                    1,
                )
            else:
                text = text[:i] + text[i:].replace(
                    token, add_color_html(token, color), 1
                )
    ...
    print(
        "Average perplexity:",
        round(np.mean(ppl_logits_processor.perplexities_list[:-1]), 4),
    )
    # t1 = time.time()
...
def ui():
    ...
    active_check = gradio.Checkbox(
        value=True,
        label="Compute probabilities and perplexity scores",
        info="Activate this extension. Note that this extension currently does not work with exllama or llama.cpp.",
    )
    color_by_ppl_check = gradio.Checkbox(
        value=False,
        label="Color by perplexity",
        info="Higher perplexity is more red. If also showing probability, higher perplexity has more blue component.",
    )
    color_by_prob_check = gradio.Checkbox(
        value=False,
        label="Color by probability",
        info="Green-yellow-red linear scale, with 100% green, 50% yellow, 0% red.",
    )
    prob_dropdown_check = gradio.Checkbox(
        value=False,
        label="Probability dropdown",
        info="Hover over a token to show a dropdown of top token probabilities. Currently slightly buggy with whitespace between tokens.",
    )
