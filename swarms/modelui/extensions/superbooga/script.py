    ...
    with gr.Row():
        with gr.Column(min_width=600):
            ...
            with gr.Tab("Generation settings"):
                ...
                gr.Markdown(
                    "Time weighting (optional, used in to make recently added chunks more likely to appear)"
                )
                time_weight = gr.Slider(
                    ...
                    label="Time weight",
                    info="Defines the strength of the time weighting. 0 = no time weighting.",
                )
                chunk_count_initial = gr.Number(
                    value=params["chunk_count_initial"],
                    label="Initial chunk count",
                    info="The number of closest-matching chunks retrieved for time weight reordering in chat mode. This should be >= chunk count. -1 = All chunks are retrieved. Only used if time_weight > 0.",
                )
