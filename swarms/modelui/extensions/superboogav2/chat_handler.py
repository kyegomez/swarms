...
def _hijack_last(context_text: str, history: dict, max_len: int, state: dict):
    ...
    for i, messages in enumerate(reversed(history["internal"])):
        for j, message in enumerate(reversed(messages)):
            num_message_tokens = get_encoded_length(
                _format_single_exchange(names[j], message)
            )
    ...
    if replace_position is None:
        logger.warn(
            "The provided context_text is too long to replace any message in the history."
        )
    else:
...
def custom_generate_chat_prompt_internal(
    user_input: str, state: dict, collector: ChromaCollector, **kwargs
):
    if parameters.get_add_chat_to_data():
        ...
        if history_as_text:
            ...
            process_and_add_to_collector(
                history_as_text, collector, False, CHAT_METADATA
            )
