...
class Handler(BaseHTTPRequestHandler):
    ...
    def _handle_get(
        ...
        if sort_param == parameters.SORT_DISTANCE:
            results = self.collector.get_sorted_by_dist(
                search_strings, n_results, max_token_count
            )
        elif sort_param == parameters.SORT_ID:
            results = self.collector.get_sorted_by_id(
                search_strings, n_results, max_token_count
            )
        ...
        try:
            ...
            if path in ["/api/v1/add", "/api/add"]:
                ...
                process_and_add_to_collector(
                    corpus, self.collector, clear_before_adding, metadata
                )
                ...
                results = self._handle_get(
                    search_strings, n_results, max_token_count, sort_param
                )
...
class APIManager:
    ...
    def start_server(self, port: int):
        ...
        self.server = CustomThreadingHTTPServer(
            (address, port), Handler, self.collector
        )
