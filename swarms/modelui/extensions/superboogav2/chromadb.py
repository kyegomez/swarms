...
class Info:
    ...
    def calculate_distance(self, other_info):
        if parameters.get_new_dist_strategy() == parameters.DIST_MIN_STRATEGY:
            ...
            return (
                2
                * (self.distance * other_info.distance)
                / (self.distance + other_info.distance)
            )
    ...
    def merge_with(self, other_info):
        ...
        if self.should_merge(s1, s2, s1_start, s2_start):
            if s1_start <= s2_start:
                if s1_start + len(s1) >= s2_start + len(
                    s2
                ):  # if s1 completely covers s2
                ...
                if s2_start + len(s2) >= s1_start + len(
                    s1
                ):  # if s2 completely covers s1
...
class ChromaCollector(Collecter):
    ...
    def add(
        ...
        with self.lock:
            assert metadatas is None or len(metadatas) == len(
                texts
            ), "metadatas must be None or have the same length as texts"
            ...
            (existing_texts, existing_embeddings, existing_ids, existing_metas), (
                ...
                non_existing_ids,
                non_existing_metas,
            ) = self._split_texts_by_cache_hit(texts, new_ids, metadatas)
            ...
            if non_existing_texts:
                non_existing_embeddings = self.embedder.embed(
                    non_existing_texts
                ).tolist()
                ...
                args = {
                    "embeddings": non_existing_embeddings,
                    "documents": non_existing_texts,
            ...
            new_info = {
                ...
                for id_, context, start_index in zip(
                    new_ids, texts_with_context, starting_indices
                )
            }
    ...
    def _split_texts_by_cache_hit(
        self, texts: list[str], new_ids: list[str], metadatas: list[dict]
    ):
    ...
    def _apply_sigmoid_time_weighing(
        self,
        infos: list[Info],
        document_len: int,
        time_steepness: float,
        time_power: float,
    ):
    ...
    def _filter_outliers_by_median_distance(
        self, infos: list[Info], significant_level: float
    ):
        ...
        filtered_infos = [
            inf for inf in infos if inf.distance <= significant_level * median_distance
        ]
    ...
    def _get_documents_ids_distances(self, search_strings: list[str], n_results: int):
        ...
        for search_string in search_strings:
            ...
            curr_infos = [
                Info(
                    start_index=self.id_to_info[id]["start_index"],
            ...
            curr_infos = self._filter_outliers_by_median_distance(
                curr_infos, parameters.get_significant_level()
            )
    ...
    def get(self, search_strings: list[str], n_results: int) -> list[str]:
        with self.lock:
            documents, _, _ = self._get_documents_ids_distances(
    ...
    # Cutoff token count
    def _get_documents_up_to_token_count(
        self, documents: list[str], max_token_count: int
    ):
        ...
        for doc in documents:
            ...
            if current_token_count + doc_token_count > max_token_count:
                ...
                truncated_doc = decode(
                    doc_tokens[:remaining_tokens], skip_special_tokens=True
                )
    ...
    def get_sorted_by_ids(
        self, search_strings: list[str], n_results: int, max_token_count: int
    ) -> list[str]:
        with self.lock:
            documents, ids, _ = self._get_documents_ids_distances(
                search_strings, n_results
            )
    ...
    def get_sorted_by_dist(
        self, search_strings: list[str], n_results: int, max_token_count: int
    ) -> list[str]:
        with self.lock:
            documents, _, distances = self._get_documents_ids_distances(
                search_strings, n_results
            )
            sorted_docs = [
                doc for doc, _ in sorted(zip(documents, distances), key=lambda x: x[1])
            ]  # sorted lowest -> highest

            # If a document is truncated or competely skipped, it would be with high distance.
            return_documents = self._get_documents_up_to_token_count(
                sorted_docs, max_token_count
            )
    ...
    def delete(self, ids_to_delete: list[str], where: dict):
        with self.lock:
            ...
            logger.info(
                f"Successfully deleted {len(ids_to_delete)} records from chromaDB."
            )
    ...
    def clear(self):
        with self.lock:
            ...
            self.collection = self.chroma_client.create_collection(
                "context", embedding_function=self.embedder.embed
            )
            self.ids = []
            self.id_to_info = {}
