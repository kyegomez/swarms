...
class TextPreprocessorBuilder:
    ...
    def to_lower(self):
        ...
        for i, token in enumerate(tokens):
            ...
            if re.match(r"^\w+$", token):
                ...
                if not re.match(r"^[A-Z]+$", token) and not re.match(
                    r"^[A-Z_]+$", token
                ):
    ...
    def num_to_char_long(self, min_len: int = 1):
        ...
        for i, token in enumerate(tokens):
            ...
            if token.isdigit() and len(token) >= min_len:
                ...
                convert_token = lambda token: "".join(
                    (chr(int(digit) + 65) * (i + 1))
                    for i, digit in enumerate(token[::-1])
                )[::-1]
    ...
    def remove_stopwords(self):
        self.text = "".join(
            [
                word
                for word in re.findall(r"\b\w+\b|\W+", self.text)
                if word not in TextPreprocessorBuilder._stop_words
            ]
        )
        return self
    ...
    def lemmatize(self):
        ...
        new_text = "".join(
            [
                TextPreprocessorBuilder._lemmatizer.lemmatize(word)
                for word in re.findall(r"\b\w+\b|\W+", self.text)
            ]
        )
...
class TextSummarizer:
    ...
    def process_long_text(text: str, min_num_sent: int) -> list[str]:
        ...
        if num_sent >= min_num_sent:
            limit_phrases = math.ceil(
                len(doc._.phrases) * 0.20
            )  # 20% of the phrases, rounded up
            limit_sentences = math.ceil(
                num_sent * 0.20
            )  # 20% of the sentences, rounded up
            result = [
                str(sent)
