...
class Parameters:
    ...
    def __init__(self):
        if Parameters._instance is not None:
            ...
            self.hyperparameters = self._load_from_json(
                Path("extensions/superboogav2/config.json")
            )
    ...
    def _load_from_json(self, file_path):
        ...
        for key in data:
            ...
            if "categories" in data[key]:
                data[key]["categories"] = [
                    self.variable_mapping.get(cat, cat)
                    for cat in data[key]["categories"]
                ]
...
def should_remove_punctuation() -> bool:
    return bool(
        Parameters.getInstance().hyperparameters["remove_punctuation"]["default"]
    )
...
def should_remove_specific_pos() -> bool:
    return bool(
        Parameters.getInstance().hyperparameters["remove_specific_pos"]["default"]
    )
...
def set_chunk_len(val: str):
    ...
    if len(chunk_lens) > 0:
        Parameters.getInstance().hyperparameters["chunk_len1"]["default"] = chunk_lens[
            0
        ]
        Parameters.getInstance().hyperparameters["chunk_len_mask"]["default"] |= 1 << 0
    if len(chunk_lens) > 1:
        Parameters.getInstance().hyperparameters["chunk_len2"]["default"] = chunk_lens[
            1
        ]
        Parameters.getInstance().hyperparameters["chunk_len_mask"]["default"] |= 1 << 1
    if len(chunk_lens) > 2:
        Parameters.getInstance().hyperparameters["chunk_len3"]["default"] = chunk_lens[
            2
        ]
        Parameters.getInstance().hyperparameters["chunk_len_mask"]["default"] |= 1 << 2
    if len(chunk_lens) > 3:
        Parameters.getInstance().hyperparameters["chunk_len4"]["default"] = chunk_lens[
            3
        ]
    ...
    if len(chunk_lens) > 4:
        logger.warning(
            f"Only up to four chunk lengths are supported. Skipping {chunk_lens[4:]}"
        )


def get_context_len() -> str:
    context_len = (
        str(Parameters.getInstance().hyperparameters["context_len_left"]["default"])
        + ","
        + str(Parameters.getInstance().hyperparameters["context_len_right"]["default"])
    )
    return context_len
...
def get_significant_level() -> float:
    return float(
        Parameters.getInstance().hyperparameters["significant_level"]["default"]
    )
...
def get_optimization_steps() -> int:
    return int(
        Parameters.getInstance().hyperparameters["optimization_steps"]["default"]
    )
