...
def output_modifier(string):
    ...
    output_file = Path(
        f"extensions/elevenlabs_tts/outputs/{wav_idx:06d}.mp3".format(wav_idx)
    )
    print(f"Outputting audio to {str(output_file)}")
    try:
        audio = elevenlabs.generate(
            text=html.unescape(string),
            voice=params["selected_voice"],
            model=params["model"],
        )
...
def ui():
    ...
    if not voices:
        ...
        if selected == "None":
            ...
            logger.error(
                f"Selected voice {selected} not available, switching to {voices[0]}"
            )
            params["selected_voice"] = voices[0]
    ...
    with gr.Row():
        voice = gr.Dropdown(
            value=params["selected_voice"], choices=voices, label="TTS Voice"
        )
    ...
    with gr.Row():
        model = gr.Dropdown(
            value=params["model"], choices=LANG_MODELS, label="Language model"
        )
