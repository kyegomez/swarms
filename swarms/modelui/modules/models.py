...
def huggingface_loader(model_name):
    ...
    config = AutoConfig.from_pretrained(
        path_to_model, trust_remote_code=params["trust_remote_code"]
    )
    ...
    # DeepSpeed ZeRO-3
        ...
        model = LoaderClass.from_pretrained(
            path_to_model, torch_dtype=params["torch_dtype"]
        )
                ...
                if params["max_memory"] is not None:
                    with init_empty_weights():
                        model = LoaderClass.from_config(
                            config, trust_remote_code=params["trust_remote_code"]
                        )
        ...
        if shared.args.disable_exllama:
            try:
                gptq_config = GPTQConfig(
                    bits=config.quantization_config.get("bits", 4), disable_exllama=True
                )
