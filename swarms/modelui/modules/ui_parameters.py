                    ...
                    with gr.Row():
                        ...
                        with gr.Column():
                            ...
                            shared.gradio["temperature_last"] = gr.Checkbox(
                                ...
                                info="Makes temperature the last sampler instead of the first.",
                            )
                            ...
                            with gr.Accordion("Other parameters", open=False):
                                ...
                                shared.gradio["num_beams"] = gr.Slider(
                                    ...
                                    label="num_beams",
                                    info="For Beam Search, along with length_penalty and early_stopping.",
                                )
                                shared.gradio["length_penalty"] = gr.Slider(
                                    -5,
                                    5,
                                    value=generate_params["length_penalty"],
                                    label="length_penalty",
                                )
                                shared.gradio["early_stopping"] = gr.Checkbox(
                                    value=generate_params["early_stopping"],
                                    label="early_stopping",
                                )

                    gr.Markdown(
                        "[Learn more](https://github.com/oobabooga/text-generation-webui/wiki/03-%E2%80%90-Parameters-Tab)"
                    )
                    ...
                    with gr.Row():
                        with gr.Column():
                            ...
                            shared.gradio["max_tokens_second"] = gr.Slider(
                                ...
                                maximum=20,
                                step=1,
                                label="Maximum number of tokens/second",
                            ...
                            shared.gradio["custom_stopping_strings"] = gr.Textbox(
                                ...
                                info='In addition to the defaults. Written between "" and separated by commas.',
                                placeholder='"\\n", "\\nYou:"',
                            )
                            shared.gradio["custom_token_bans"] = gr.Textbox(
                                value=shared.settings["custom_token_bans"] or None,
                                label="Custom token bans",
                                info="Specific token IDs to ban from generating, comma-separated. The IDs can be found in the Default or Notebook tab.",
                            )
...
def create_event_handlers():
    ...
    shared.gradio["grammar_file"].change(
        load_grammar, gradio("grammar_file"), gradio("grammar_string")
    )


def get_truncation_length():
    if (
        "max_seq_len" in shared.provided_arguments
        or shared.args.max_seq_len != shared.args_defaults.max_seq_len
    ):
        return shared.args.max_seq_len
    elif (
        "n_ctx" in shared.provided_arguments
        or shared.args.n_ctx != shared.args_defaults.n_ctx
    ):
