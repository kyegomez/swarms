                ...
                with gr.Row():
                    with gr.Column():
                        with gr.Row():
                            ...
                            shared.gradio["reload_model"] = gr.Button(
                                "Reload",
                                elem_classes="refresh-button",
                                interactive=not mu,
                            )
        ...
        with gr.Row():
            with gr.Column():
                shared.gradio["loader"] = gr.Dropdown(
                    label="Model loader",
                    choices=loaders.loaders_and_params.keys(),
                    value=None,
                )
                with gr.Box():
                    with gr.Row():
                        with gr.Column():
                            ...
                            shared.gradio["cpu_memory"] = gr.Slider(
                                label="cpu-memory in MiB",
                                maximum=total_cpu_mem,
                                value=default_cpu_mem,
                            )
                            shared.gradio["transformers_info"] = gr.Markdown(
                                "load-in-4bit params:"
                            )
                            ...
                            shared.gradio["n_ctx"] = gr.Slider(
                                ...
                                info="Context length. Try lowering this if you run out of memory while loading the model.",
                            )
                            ...
                            shared.gradio["rope_freq_base"] = gr.Slider(
                                label="rope_freq_base",
                                minimum=0,
                                maximum=1000000,
                                step=1000,
                                info="If greater than 0, will be used instead of alpha_value. Those two are related by rope_freq_base = 10000 * alpha_value ^ (64 / 63)",
                        ...
                        with gr.Column():
                            ...
                            shared.gradio["no_inject_fused_attention"] = gr.Checkbox(
                                ...
                                info="Disable fused attention. Fused attention improves inference performance but uses more VRAM. Fuses layers for AutoAWQ. Disable if running low on VRAM.",
                            )
                            ...
                            shared.gradio["no_mul_mat_q"] = gr.Checkbox(
                                label="no_mul_mat_q",
                                value=shared.args.no_mul_mat_q,
                                info="Disable the mulmat kernels.",
                            )
                            ...
                            shared.gradio["tensor_split"] = gr.Textbox(
                                label="tensor_split",
                                info="Split the model across multiple GPUs, comma-separated list of proportions, e.g. 18,17",
                            )
                            ...
                            shared.gradio["trust_remote_code"] = gr.Checkbox(
                                ...
                                info="To enable this option, start the web UI with the --trust-remote-code flag. It is necessary for some models.",
                                interactive=shared.args.trust_remote_code,
                            )
                            shared.gradio["cfg_cache"] = gr.Checkbox(
                                label="cfg-cache",
                                value=shared.args.cfg_cache,
                                info="Create an additional cache for CFG negative prompts.",
                            )
                            shared.gradio["logits_all"] = gr.Checkbox(
                                label="logits_all",
                                value=shared.args.logits_all,
                                info="Needs to be set for perplexity evaluation to work. Otherwise, ignore it, as it makes prompt processing slower.",
                            )
                            shared.gradio["use_flash_attention_2"] = gr.Checkbox(
                                label="use_flash_attention_2",
                                value=shared.args.use_flash_attention_2,
                                info="Set use_flash_attention_2=True while loading the model.",
                            )
                            shared.gradio["disable_exllama"] = gr.Checkbox(
                                label="disable_exllama",
                                value=shared.args.disable_exllama,
                                info="Disable ExLlama kernel.",
                            )
                            shared.gradio["no_flash_attn"] = gr.Checkbox(
                                label="no_flash_attn",
                                value=shared.args.no_flash_attn,
                                info="Force flash-attention to not be used.",
                            )
                            shared.gradio["cache_8bit"] = gr.Checkbox(
                                label="cache_8bit",
                                value=shared.args.cache_8bit,
                            ...
                            shared.gradio["gptq_for_llama_info"] = gr.Markdown(
                                "GPTQ-for-LLaMa support is currently only kept for compatibility with older GPUs. AutoGPTQ or ExLlama is preferred when compatible. GPTQ-for-LLaMa is installed by default with the webui on supported systems. Otherwise, it has to be installed manually following the instructions here: [instructions](https://github.com/oobabooga/text-generation-webui/blob/main/docs/GPTQ-models-(4-bit-mode).md#installation-1)."
                            )
                            shared.gradio["exllama_info"] = gr.Markdown(
                                "For more information, consult the [docs](https://github.com/oobabooga/text-generation-webui/wiki/04-%E2%80%90-Model-Tab#exllama_hf)."
                            )
...
def create_event_handlers():
    shared.gradio["loader"].change(
        ...
        lambda value: gr.update(choices=loaders.get_model_types(value)),
        gradio("loader"),
        gradio("model_type"),
    )
    ...
    shared.gradio["model_menu"].change(
        ...
        update_truncation_length,
        gradio("truncation_length", "interface_state"),
    ...
    shared.gradio["reload_model"].click(unload_model, None, None).then(
        ui.gather_interface_values,
        gradio(shared.input_elements),
        gradio("interface_state"),
    ).then(update_model_parameters, gradio("interface_state"), None).then(
    ...
    shared.gradio["unload_model"].click(unload_model, None, None).then(
...
def download_model_wrapper(
    ...
    try:
        ...
        output_folder = downloader.get_output_folder(
            model, branch, is_lora, is_llamacpp=is_llamacpp, base_folder=base_folder
        )
        if check:
            progress(0.5)
            ...
            yield (
                f"Downloading file{'s' if len(links) > 1 else ''} to `{output_folder}/`"
            )
