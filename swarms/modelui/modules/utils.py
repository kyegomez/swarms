...
def get_available_models():
    ...
    for item in list(Path(f"{shared.args.model_dir}/").glob("*")):
        if (
            not item.name.endswith((".txt", "-np", ".pt", ".json", ".yaml", ".py"))
...
def get_available_characters():
    paths = (
        x
        for x in Path("swarms/modelui/characters").iterdir()
...
def get_available_grammars():
    return ["None"] + sorted(
        [item.name for item in list(Path("grammars").glob("*.gbnf"))], key=natural_keys
    )
