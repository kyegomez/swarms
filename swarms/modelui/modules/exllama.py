...
try:
    ...
    logger.warning(
        "exllama module failed to import. Will attempt to import from repositories/."
    )
    try:
...
class ExllamaModel:
    ...
    def encode(self, string, **kwargs):
        return self.tokenizer.encode(
            string, max_seq_len=self.model.config.max_seq_len, add_bos=True
        )
    ...
    def get_logits(self, token_ids, **kwargs):
        ...
        if token_ids.shape[-1] > 1:
            self.model.forward(
                token_ids[:, :-1], self.cache, input_mask=None, preprocess_only=True
            )
    ...
    def generate_with_streaming(self, prompt, state):
        ...
        if state["guidance_scale"] == 1:
            ...
            ids = self.generator.tokenizer.encode(
                prompt, max_seq_len=self.model.config.max_seq_len
            )
            if state["add_bos_token"]:
                ids = torch.cat(
                    [torch.tensor([[self.tokenizer.bos_token_id]]).to(ids.device), ids],
                    dim=1,
                ).to(torch.int64)
        ...
        # Copied from https://github.com/turboderp/exllama/blob/master/example_cfg.py
            ...
            for i in range(max_new_tokens):
                logits = self.model.forward(
                    self.generator.sequence[:, -1:], self.cache, input_mask=mask
                )
