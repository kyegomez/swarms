            ...
            with gr.Row():
                with gr.Column():
                    ...
                    with gr.Accordion(label="Target Modules", open=False):
                        gr.Markdown(
                            "Selects which  swarms.modelui.modules to target in training. Targeting more  swarms.modelui.modules is closer to a full fine-tune at the cost of increased VRAM requirements and adapter size.\nNOTE: Only works for model_id='llama', other types will retain default training behavior and not use these settings."
                        )
                        with gr.Row():
                            with gr.Column():
