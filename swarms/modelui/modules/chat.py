    ...
    if is_instruct:
        ...
        if state["custom_system_message"].strip() != "":
            context = context.replace(
                "<|system-message|>", state["custom_system_message"]
            )
        else:
...
def chatbot_wrapper(
    ...
    for j, reply in enumerate(
        generate_reply(prompt, state, stopping_strings=stopping_strings, is_chat=True)
    ):
        # Extract the reply
        visible_reply = reply
        if state["mode"] in ["chat", "chat-instruct"]:
...
def load_character(character, name1, name2, instruct=False):
    ...
    if filepath is None or not filepath.exists():
        logger.error(
            f'Could not find the character "{character}" inside {folder}/. No character has been loaded.'
        )
        raise ValueError
...
def generate_instruction_template_yaml(
    user, bot, context, turn_template, system_message
):
    data = {
        "user": user,
        "bot": bot,
