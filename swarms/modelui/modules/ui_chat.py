...
def create_ui():
    ...
    with gr.Tab(
        "Chat",
        elem_id="chat-tab",
        elem_classes=("old-ui" if shared.args.chat_buttons else None),
    ):
        with gr.Row():
            with gr.Column(elem_id="chat-col"):
                shared.gradio["display"] = gr.HTML(
                    value=chat_html_wrapper(
                        {"internal": [], "visible": []}, "", "", "chat", "cai-chat"
                    )
                )
        ...
        with gr.Column(elem_id="chat-buttons"):
            with gr.Row():
                ...
                shared.gradio["Remove last"] = gr.Button(
                    "Remove last reply (Ctrl + Shift + Backspace)",
                    elem_id="Remove-last",
                )

            with gr.Row():
                shared.gradio["Replace last reply"] = gr.Button(
                    "Replace last reply (Ctrl + Shift + L)", elem_id="Replace-last"
                )
                shared.gradio["Copy last reply"] = gr.Button(
                    "Copy last reply (Ctrl + Shift + K)", elem_id="Copy-last"
                )
        ...
        with gr.Row(elem_id="past-chats-row"):
            shared.gradio["unique_id"] = gr.Dropdown(
                label="Past chats", elem_classes=["slim-dropdown"], interactive=not mu
            )
            ...
            shared.gradio["delete_chat-cancel"] = gr.Button(
                "Cancel", visible=False, elem_classes="refresh-button"
            )
            shared.gradio["delete_chat-confirm"] = gr.Button(
                "Confirm", variant="stop", visible=False, elem_classes="refresh-button"
            )
        ...
        with gr.Row(elem_id="rename-row"):
            ...
            shared.gradio["rename_to-confirm"] = gr.Button(
                "Confirm", visible=False, elem_classes="refresh-button"
            )
        ...
        with gr.Row():
            shared.gradio["mode"] = gr.Radio(
                ...
                value="chat",
                label="Mode",
                info="Defines how the chat prompt is generated. In instruct and chat-instruct modes, the instruction template selected under Parameters > Instruction template must match the current model.",
                elem_id="chat-mode",
            )
...
def create_chat_settings_ui():
        ...
        with gr.Row():
            with gr.Column(scale=8):
                ...
                shared.gradio["name2"] = gr.Textbox(
                    value="", lines=1, label="Character's name"
                )
            ...
            with gr.Column(scale=1):
                shared.gradio["character_picture"] = gr.Image(
                    label="Character picture", type="pil", interactive=not mu
                )
    ...
    with gr.Tab("Instruction template"):
        ...
        shared.gradio["name1_instruct"] = gr.Textbox(
            ...
            lines=2,
            label="User string",
            info="Replaces <|user|> in the turn template.",
        )
        shared.gradio["name2_instruct"] = gr.Textbox(
            value="",
            lines=1,
            label="Bot string",
            info="Replaces <|bot|> in the turn template.",
        )
        shared.gradio["context_instruct"] = gr.Textbox(
            value="", lines=4, label="Context", elem_classes=["add_scrollbar"]
        )
        shared.gradio["system_message"] = gr.Textbox(
            value="",
            lines=2,
            label="System message",
            info="Replaces <|system-message|> in the context.",
            elem_classes=["add_scrollbar"],
        )
        with gr.Row():
            shared.gradio["send_instruction_to_default"] = gr.Button(
            ...
            shared.gradio["send_instruction_to_negative_prompt"] = gr.Button(
...
def create_event_handlers():
    ...
    if not shared.args.multi_user:
        shared.gradio["unique_id"].select(
            chat.load_history,
            gradio("unique_id", "character_menu", "mode"),
            gradio("history"),
        ).then(chat.redraw_html, gradio(reload_arr), gradio("display"))

    shared.gradio["Start new chat"].click(
        ui.gather_interface_values,
        gradio(shared.input_elements),
        gradio("interface_state"),
    ).then(chat.start_new_chat, gradio("interface_state"), gradio("history")).then(
        chat.redraw_html, gradio(reload_arr), gradio("display")
    ).then(
        lambda x: gr.update(
            choices=(histories := chat.find_all_histories(x)), value=histories[0]
        ),
        gradio("interface_state"),
        gradio("unique_id"),
    )
    ...
    shared.gradio["delete_chat-confirm"].click(
        ...
        chat.delete_history, gradio("unique_id", "character_menu", "mode"), None
    ).then(
    ...
    shared.gradio["rename_chat"].click(
        lambda x: x, gradio("unique_id"), gradio("rename_to")
    ).then(
        lambda: [gr.update(visible=True)] * 3,
        None,
        gradio("rename_to", "rename_to-confirm", "rename_to-cancel"),
        show_progress=False,
    )
    ...
    shared.gradio["rename_to-confirm"].click(
        chat.rename_history,
        gradio("unique_id", "rename_to", "character_menu", "mode"),
        None,
    ).then(
        lambda: [gr.update(visible=False)] * 3,
        None,
        ...
        lambda x, y: gr.update(choices=chat.find_all_histories(x), value=y),
        gradio("interface_state", "rename_to"),
        gradio("unique_id"),
    )
    ...
    shared.gradio["load_chat_history"].upload(
        ...
        gradio("load_chat_history", "history"),
        gradio("history"),
    ).then(
        chat.redraw_html, gradio(reload_arr), gradio("display")
    ).then(
    ...
    shared.gradio["mode"].change(
        ...
        partial(chat.character_is_loaded, raise_exception=True),
        gradio("interface_state"),
        None,
    ).success(
        chat.load_latest_history, gradio("interface_state"), gradio("history")
    ...
    ).then(lambda: "instruction-templates/", None, gradio("save_root")).then(
    ...
    ).then(lambda: "instruction-templates/", None, gradio("delete_root")).then(
    ...
    shared.gradio["save_chat_history"].click(
        ...
        None,
        _js=f"(hist, char, mode) => {{{ui.save_files_js}; saveHistory(hist, char, mode)}}",
    )
    ...
    shared.gradio["Submit character"].click(
        ...
        None,
        None,
        _js=f"() => {{{ui.switch_tabs_js}; switch_to_character()}}",
    )
    ...
    shared.gradio["send_instruction_to_default"].click(
        ...
        lambda x: x.update(
            {"mode": "instruct", "history": {"internal": [], "visible": []}}
        ),
        gradio("interface_state"),
        None,
    ).then(
        partial(chat.generate_chat_prompt, "Input"),
        gradio("interface_state"),
        ...
        None,
        None,
        _js=f"() => {{{ui.switch_tabs_js}; switch_to_default()}}",
    )
    ...
    shared.gradio["send_instruction_to_notebook"].click(
        ...
        None,
        None,
        _js=f"() => {{{ui.switch_tabs_js}; switch_to_notebook()}}",
    )
    ...
    shared.gradio["send_instruction_to_negative_prompt"].click(
        ...
        partial(chat.generate_chat_prompt, "Input"),
        gradio("interface_state"),
        gradio("negative_prompt"),
    ).then(
        lambda: None,
        None,
        None,
        _js=f"() => {{{ui.switch_tabs_js}; switch_to_generation_parameters()}}",
    )
    ...
    shared.gradio["send-chat-to-default"].click(
        ...
        partial(chat.generate_chat_prompt, "", _continue=True),
        gradio("interface_state"),
        gradio("textbox-default"),
    ).then(
        lambda: None,
