    ...
    if model_settings["loader"] in ["llama.cpp", "llamacpp_HF", "ctransformers"]:
        ...
        if path.exists():
            ...
            if "max_position_embeddings" in metadata:
                model_settings["truncation_length"] = metadata[
            ...
            if (
                ...
                and type(metadata["rope_scaling"]) is dict
            ...
            if "quantization_config" in metadata:
                ...
                if "group_size" in metadata["quantization_config"]:
                    model_settings["groupsize"] = metadata["quantization_config"][
                        "group_size"
                    ]
                if "desc_act" in metadata["quantization_config"]:
                    model_settings["desc_act"] = metadata["quantization_config"][
                        "desc_act"
                    ]
...
def infer_loader(model_name, model_settings):
    ...
    if not path_to_model.exists():
        ...
        loader = "ExLlama_HF"
    elif (path_to_model / "quant_config.json").exists() or re.match(
        r".*-awq", model_name.lower()
    ):
        loader = "AutoAWQ"
...
def apply_model_settings_to_state(model, state):
    ...
    if "loader" in model_settings:
        ...
        if not (
            loader == "AutoGPTQ"
            and state["loader"]
            in ["GPTQ-for-LLaMa", "ExLlama", "ExLlama_HF", "ExLlamav2", "ExLlamav2_HF"]
        ) and not (
            loader == "llama.cpp"
            and state["loader"] in ["llamacpp_HF", "ctransformers"]
        ):
            state["loader"] = loader
