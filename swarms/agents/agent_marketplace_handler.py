...
class AgentMarketplaceHandler:
    ...
    def load_prompt(self, prompt_id: Optional[str] = None) -> None:
        ...
        # Names are generated per instance now, so equality against a literal
        # would never match; the agent records whether it named itself.
        if name and getattr(agent, "name_autogenerated", False):
