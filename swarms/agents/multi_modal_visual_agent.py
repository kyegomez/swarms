...
class ImageCaptioning:
    ...
    def inference(self, image_path):
        inputs = self.processor(Image.open(image_path),
...
class CannyText2Image:
    ...
    def inference(self, inputs):
        ...
        updated_image_path = get_new_image_name(image_path,
                                                func_name="canny2image")
        image.save(updated_image_path)
        print(
...
class Image2Hed:
    ...
    def inference(self, inputs):
        ...
        updated_image_path = get_new_image_name(inputs,
                                                func_name="hed-boundary")
...
class Segmenting:
    ...
    def segment_image_with_boxes(self, image_pil, image_path, boxes_filt,
        ...
        for mask in masks:
            image = self.show_mask(mask[0].cpu().numpy(),
                                   image,
                                   random_color=True,
                                   transparency=0.3)
...
class InfinityOutPainting:
    ...
    def get_BLIP_vqa(self, image, question):
        inputs = self.ImageVQA.processor(image, question,
                                         return_tensors="pt").to(
                                             self.ImageVQA.device,
                                             self.ImageVQA.torch_dtype)
        out = self.ImageVQA.model.generate(**inputs)
        answer = self.ImageVQA.processor.decode(out[0],
    ...
    def resize_image(self, image, max_size=1000000, multiple=8):
        ...
        new_width, new_height = new_width - (
            new_width % multiple), new_height - (new_height % multiple)
    ...
    def dowhile(self, original_img, tosize, expand_ratio, imagine, usr_prompt):
        ...
        while old_img.size != tosize:
            prompt = (self.check_prompt(usr_prompt) if usr_prompt else
                      self.get_imagine_caption(old_img, imagine))
            ...
            temp_canvas_size = (
                ...
                if expand_ratio * old_img.width < tosize[0] else tosize[0],
                expand_ratio * old_img.height
                if expand_ratio * old_img.height < tosize[1] else tosize[1],
            )
            temp_canvas, temp_mask = Image.new("RGB",
                                               temp_canvas_size,
                                               color="white"), Image.new(
                                                   "L",
                                                   temp_canvas_size,
                                                   color="white")
...
class ObjectSegmenting:
    ...
    def get_mask(self, image_path, text_prompt):
        ...
        for mask in masks:
            image = self.sam.show_mask(mask[0].cpu().numpy(),
                                       image,
                                       random_color=True,
                                       transparency=0.3)
...
class MultiModalVisualAgent:
    ...
    def init_agent(self, lang):
        ...
        if lang == "English":
            ...
            PREFIX, FORMAT_INSTRUCTIONS, SUFFIX = (
                VISUAL_AGENT_PREFIX_CN,
                VISUAL_AGENT_FORMAT_INSTRUCTIONS_CN,
                VISUAL_AGENT_SUFFIX_CN,
            )
    ...
    def run_image(self, image, lang):
        ...
        self.agent.memory.buffer = (self.agent.memory.buffer + Human_prompt +
                                    "AI: " + AI_prompt)
...
class MultiModalAgent:
    ...
    def __init__(self,
        ...
        self.agent = MultiModalVisualAgent(load_dict, temperature)
        self.language = language
        self.history = []

    def run_text(self, text: str = None, language="english"):
    ...
    def run_img(self, image_path: str, language="english"):
        """If language is None"""
        if language is None:
            language = self.default_language

        try:
            return self.agent.run_image(image_path, language)
