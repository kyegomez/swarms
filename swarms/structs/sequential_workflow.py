...
<<<<<<< HEAD
        flow (Union[Callable, Agent]): The model or flow to execute the task.
=======
...
<<<<<<< HEAD
    ...
    >>> flow = Agent(llm=OpenAIChat(openai_api_key=""), max_loops=1, dashboard=False)
=======
...
>>>>>>> 3d3dddaf0c7894ec2df14c51f7dd843c41c878c4
        ...
        """
<<<<<<< HEAD
        # If the flow is a Agent instance, we include the task in kwargs for Agent.run()
        if isinstance(flow, Agent):
...
<<<<<<< HEAD
                        # Check if the flow is a Agent and a 'task' argument is needed
                        if isinstance(task.flow, Agent):
=======
...
<<<<<<< HEAD
                            # If it's not a Agent instance, call the flow directly
=======
...
<<<<<<< HEAD
                            if isinstance(next_task.flow, Agent):
