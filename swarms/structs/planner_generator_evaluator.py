...
class PlannerGeneratorEvaluator:
    ...
    def _run_planner(self, prompt: str) -> str:
        ...
        """
        planner_task = (
            "Read the conversation above and create a comprehensive plan "
            "for the user's prompt.\n\n"
            "Generate a detailed plan with steps and evaluation criteria. "
            "Write your plan in the structured format specified in your instructions."
        )

        plan = self.planner_agent.run(
            task=planner_task,
            messages=messages_for(
                self.planner_agent.agent_name, self.conversation
            ),
        )
    ...
    def _negotiate_contract(self, step_number: int) -> StepContract:
        ...
        generator_task = (
            f"Read the conversation above and propose a step contract for Step {step_number}.\n\n"
        ...
        contract_proposal = self.generator_agent.run(
            task=generator_task,
            messages=messages_for(
                self.generator_agent.agent_name, self.conversation
            ),
        )
        ...
        contract_review = self.evaluator_agent.run(
            task=evaluator_task,
            messages=messages_for(
                self.evaluator_agent.agent_name, self.conversation
            ),
        )
        self._append_to_shared_state(
    ...
    def _execute_step(
        ...
        output = self.generator_agent.run(
            task=generator_task,
            messages=messages_for(
                self.generator_agent.agent_name, self.conversation
            ),
