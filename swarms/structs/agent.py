...
AGENT_NAME_ALPHABET = string.ascii_lowercase + string.digits


def generate_agent_name() -> str:
    """A unique default agent name, e.g. ``agent-k3f9q``.

    Must be called per instance, never used as a signature default: Python
    evaluates a default argument once at import, so a generated default
    would bake a single name into the signature and hand it to every agent
    -- the defect ``test_component_ids.py`` pins for component ids.

    Returns:
        ``agent-`` followed by five random lowercase alphanumerics.
    """
    suffix = "".join(
        secrets.choice(AGENT_NAME_ALPHABET) for _ in range(5)
    )
...
class Agent:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.name_autogenerated = agent_name is None
        ...
        # Dynamic tools, MCP schemas, and autonomous loop options setup, with MCP functionality managed by MCPManager.
